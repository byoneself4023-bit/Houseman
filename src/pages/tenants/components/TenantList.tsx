          ...
          <thead>
            <tr className="border-b-2 border-hm-border">
              {["유형","건물명","호실","입주자","연락처","입주일","만기일","보증금","월세","관리비","상태","청구\u2460","청구\u2461","청구\u2462"].map((h, i) => (
                <th key={i} className={`py-2.5 text-xs font-bold text-hm-text-muted whitespace-nowrap ${
            ...
            {visibleFiltered.map((r, i) => {
              ...
              return (
                <tr key={i} onClick={() => setSelectedTenant(r)}
                  ...
                  <td className="py-2.5 px-0.5 text-xs text-hm-text-sub">{r.phone}</td>
                  <td className="py-2.5 px-0.5 text-xs">{r.moveIn ? r.moveIn.slice(2) : "-"}</td>
                  <td className="py-2.5 px-1 text-xs">{r.expiry ? r.expiry.slice(2) : "-"}</td>
                  <td className="py-2.5 px-1 text-right text-xs">{fmt(r.deposit)}</td>
                  <td className="py-2.5 px-1 text-right text-xs">{fmt(r.rent)}</td>
                  <td className="py-2.5 px-1 text-right text-xs text-hm-text-muted">{fmt(r.mgmt)}</td>
                  ...
                  {[0, 1, 2].map(si => (
                    <td key={si} className="py-2.5 px-2.5 text-right text-xs">
                      {slots[si] ? <><span className={`font-bold ${slotColors[si]}`}>{fmt(slots[si].amount)}</span>{(slots[si] as any).lateFee > 0 && <div className="text-xs text-hm-danger font-semibold">연체료 {fmt((slots[si] as any).lateFee)}</div>}</> : <span className="text-gray-300">&mdash;</span>}
                    </td>
                  ))}
                </tr>
              );
      ...
      {visibleCount < filtered.length && (
        <div className="text-center py-4">
          <button onClick={() => setVisibleCount(v => v + 100)}
            className="py-2.5 px-8 rounded-lg border border-hm-input-border bg-white text-sm font-bold cursor-pointer font-[inherit] text-hm-text-sub hover:bg-hm-bg-hover transition-colors">
