...
export const ParkingPage = ({ myBuildings = [], activeTenants = [], parkingInfo, setParkingInfo, isLoading }: ParkingPageProps) => {
  ...
  const columns = [
    ...
    { label: "입주자", key: "name", width: "3%", render: (row: Record<string, any>) => <span className="text-[11px] truncate block max-w-[90px]" title={row.name}>{row.name}</span> },
  ...
  return (
    <div>
      ...
      {displayList.length === 0 ? (
        ...
        <Card className="overflow-auto max-w-[900px]">
