      ...
      {/* ========== 담당자 ========== */}
        <div>
            ...
            {staffRoles.map(role => {
              ...
              return (
                <div key={role.id} onClick={() => setFilterRole(active ? "all" : role.id)}
                  ...
                  <div className={`text-sm font-bold ${active ? 'text-white' : 'text-hm-text'}`}>{role.label}</div>
                  <div className="text-xl font-bold mt-1" style={{ color: active ? "#fff" : role.color }}>{count}<span className="text-xs font-semibold">명</span></div>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex justify-between items-center mb-3">
            <div className="text-sm font-bold">{filterRole === "all" ? "전체 담당자" : `${roleOf(filterRole).icon} ${roleOf(filterRole).label} 담당자`} ({filteredStaff.length}명)</div>
          ...
          {showAdd && (
            <Card style={{ marginBottom: 16, border: "2px solid var(--color-hm-blue)" }}>
              <div className="text-xs font-bold text-hm-blue-dark mb-3">새 담당자 등록</div>
              <div className="grid grid-cols-3 gap-3 mb-3">
                <div>
                  <div className="text-xs text-hm-text-muted mb-[3px]">이름 *</div>
                  <input value={newStaff.name} onChange={e => setNewStaff(p => ({ ...p, name: e.target.value }))} placeholder="담당자명" className={`${inputClassName} py-2`} />
                </div>
                <div>
                  <div className="text-xs text-hm-text-muted mb-[3px]">연락처</div>
                ...
                <div className="flex gap-1 flex-wrap">
                  {staffRoles.map(r => {
                    ...
                    return <button key={r.id} onClick={() => setNewStaff(p => ({ ...p, roles: sel ? p.roles.filter(x => x !== r.id) : [...p.roles, r.id] }))}
                      className="px-3 py-1.5 rounded-md text-xs cursor-pointer font-[inherit] transition-colors"
              ...
              <button onClick={addStaff} className={`px-7 py-2.5 rounded-lg border-none text-white font-bold text-sm font-[inherit] ${newStaff.name && newStaff.roles.length > 0 ? 'bg-hm-blue-dark cursor-pointer hover:bg-blue-700' : 'bg-gray-300 cursor-default'} transition-colors`}>등록</button>
            </Card>
          )}
                    ...
                    <div className="flex items-center gap-3.5">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-base font-bold text-hm-text">{s.name}</span>
                          {s.roles.map(rid => {
                            const r = roleOf(rid);
                            return <span key={rid} className="text-xs px-2 py-0.5 rounded-[5px] font-bold" style={{ background: `${r.color}15`, color: r.color }}>{r.icon} {r.label}</span>;
                          })}
                        </div>
                  ...
                  {editingId === s.id && editStaff && (
                    <div className="mt-3 pt-3 border-t-2 border-hm-blue">
                      ...
                      <div className="flex gap-2">
                        <button onClick={saveEdit} className={`px-6 py-2 rounded-lg border-none text-white font-bold text-xs font-[inherit] ${editStaff.name && editStaff.roles.length > 0 ? 'bg-hm-blue-dark cursor-pointer hover:bg-blue-700' : 'bg-gray-300 cursor-default'} transition-colors`}>저장</button>
