...
export const RenewalPage = ({ myBuildings = [], activeTenants = [] }) => {
  ...
  const [renewalNotes, setRenewalNotes] = useLocalStorage("hm_renewalNotes", {});
  ...
  // 재계약 완료 목록 (만기 연장된 호실)
  const [renewedRooms, setRenewedRooms] = useLocalStorage("hm_renewedRooms", {});

  const today = new Date();

  // 묵시적 갱신: 만기가 지났으면 매년 같은 월/일이 재계약 시점
  // 다음 돌아오는 월/일까지 남은 일수를 반환
  const getNextRenewalDate = (t) => {
  ...
  const tenants = useMemo(() => {
    ...
    return base
      ...
      .map(t => {
        ...
        return { ...t, daysLeft, renewalDate: date, tacit };
      })
  ...
  const getExpiryLabel = (daysLeft, tacit) => {
  ...
  const isTodayFn = (d) => todayMonStr === calMonStr && d === todayDate;
  ...
  // 요청사항 입력 폼 열기 (팝업)
  const openNoteForm = (rk, building, room) => {
          ...
          {calDays.map((day, i) => {
            ...
            return (
              <div key={i}
                ...
                onMouseLeave={e => !selected && !isDrop && (e.currentTarget.style.background = isTodayFn(day) ? "#FFFBEB" : "transparent")}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  ...
                  {noteItems.slice(0, 2).map((n, ni) => (
                    <div key={`n${ni}`}
                      ...
                      onDragStart={e => { e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain", n.rk); e.currentTarget.style.opacity = "0.4"; }}
                      onDragEnd={e => { e.currentTarget.style.opacity = "1"; setDropTargetDay(null); }}
              ...
              {selectedDayNotes.map((n, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", borderRadius: 8, background: n.done ? "#ECFDF5" : "#F5F3FF", border: `1px solid ${n.done ? "#A7F3D0" : "#DDD6FE"}`, marginBottom: 4 }}>
                  ...
                  <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); deleteNote(n.rk); }}
              ...
              {filtered.map((t, i) => {
                ...
                return (
                  <tr key={i} style={{ borderBottom: "1px solid #F0F2F5" }}
                    ...
                    <td style={{ padding: "4px 2px", minWidth: 160 }} onClick={e => e.stopPropagation()}>
                      {hasNote ? (
                        <div onClick={() => openNoteForm(rk, t.building, t.room)}
                          style={{ padding: "4px 6px", fontSize: 10, cursor: "pointer", borderRadius: 4, textDecoration: data.done ? "line-through" : "none", color: data.done ? "#059669" : "#1A1D23" }}>
                          <span style={{ color: "#7C3AED", fontWeight: 700, marginRight: 4 }}>{data.date?.slice(5)} {data.time}</span>
                          {data.note}
                        </div>
                      ) : (
                        <div onClick={() => openNoteForm(rk, t.building, t.room)}
                    ...
                    <td style={{ padding: "4px 2px", textAlign: "center" }} onClick={e => e.stopPropagation()}>
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                        {hasNote && (
                          <input type="checkbox" checked={data.done || false}
