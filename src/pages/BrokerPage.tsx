  ...
  return (
    <div>
      ...
      {collectedBrokers.length > 0 && (
        <div className="mb-5">
          <div className="text-sm font-bold text-gray-500 mb-2">계약 이력에서 수집 ({collectedBrokers.length}개)</div>
              ...
              return (
                <div key={i} className={`flex items-center justify-between px-3.5 py-2.5 rounded-lg border ${alreadyAdded ? 'bg-green-50 border-green-200' : 'bg-hm-bg-hover border-gray-200'}`}>
                  <div>
                    ...
                    {b.buildings.length > 0 && <span className="text-xs text-gray-400 ml-2">{b.buildings.join(", ")}</span>}
                  </div>
                  {alreadyAdded
      ...
      <div className="flex justify-between items-center mb-3">
        <div className="text-sm font-bold">등록 부동산 ({brokerList.length}개)</div>
      ...
      {showAdd && (
        <Card style={{ marginBottom: 16, border: "2px solid var(--color-hm-blue)" }}>
          <div className="text-xs font-bold text-hm-blue-dark mb-3">새 부동산 등록</div>
          <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-3'} gap-3 mb-3`}>
            <div>
              <div className="text-xs text-hm-text-muted mb-[3px]">부동산명 *</div>
            ...
            <div>
              <div className="text-xs text-hm-text-muted mb-[3px]">담당 지역/건물</div>
              <input value={newBroker.area} onChange={e => setNewBroker(p => ({ ...p, area: e.target.value }))} placeholder="관악구" className={`${inputClassName} py-2`} />
            </div>
          </div>
          <button onClick={addBroker} className={`px-7 py-2.5 rounded-lg border-none text-white font-bold text-sm font-[inherit] ${newBroker.name && newBroker.phone ? 'bg-hm-blue-dark cursor-pointer hover:bg-blue-700' : 'bg-gray-300 cursor-default'} transition-colors`}>등록</button>
        </Card>
      )}
      ...
      <div className="flex flex-col gap-2">
          ...
          <Card key={b.id} className="!py-3.5 !px-[18px]">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <div className="text-base font-bold text-hm-text">{b.name}</div>
            ...
            {editingId === b.id && editBroker && (
              <div className="mt-3 pt-3 border-t-2 border-pink-500">
                <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-3'} gap-3 mb-3`}>
                  ...
                  <div>
                    <div className="text-xs text-hm-text-muted mb-[3px]">담당 지역</div>
                    <input value={editBroker.area} onChange={e => setEditBroker(p => ({ ...p, area: e.target.value }))} className={`${inputClassName} py-2`} />
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={saveEdit} className="px-6 py-2 rounded-lg border-none bg-pink-500 text-white font-bold text-xs cursor-pointer font-[inherit] hover:bg-pink-600 transition-colors">저장</button>
        ...
        {brokerList.length === 0 && (
          <div className="py-8 text-center text-gray-400 text-sm">등록된 부동산이 없습니다. 위에서 추가하거나 계약 이력에서 가져오세요.</div>
        )}
      </div>
    </div>
  );
