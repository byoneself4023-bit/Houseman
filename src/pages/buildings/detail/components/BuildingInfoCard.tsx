      ...
      <Card className="mb-4">
        <div className={`flex justify-between items-center ${sec1Open ? 'mb-3' : ''}`}>
          <div onClick={() => setSec1Open(!sec1Open)} className="cursor-pointer flex-1">
            <div className="text-base font-bold text-hm-text">📋 기본 정보</div>
            <div className="text-xs text-hm-text-muted mt-0.5">건물 유형 · 주소 · 시설 · 관리 시작일</div>
          </div>
              ...
              <div className="flex flex-wrap gap-1.5 items-center">
                  ...
                  return (
                    <div key={ti} className="flex items-center gap-1">
                      <select value={t} onChange={e => { const updated = [...detailBuildingTypes]; updated[ti] = e.target.value; setDetailBuildingTypes(updated); }}
                        className={`${inputClassName} !px-2 !py-[5px] !text-xs !font-bold !cursor-pointer`}
                      ...
                      {detailBuildingTypes.length > 1 && (
                        <button onClick={() => setDetailBuildingTypes(detailBuildingTypes.filter((_, i) => i !== ti))}
                          className="w-5 h-5 rounded-[5px] border border-red-300 bg-hm-danger-bg text-hm-danger cursor-pointer text-xs flex items-center justify-center p-0 font-[inherit] hover:brightness-90 transition-all">✕</button>
                      )}
                    </div>
                  );
                })}
                {detailBuildingTypes.length < 3 && (
                  <button onClick={() => { const remaining = BUILDING_TYPES.filter(bt => !detailBuildingTypes.includes(bt)); if (remaining.length > 0) setDetailBuildingTypes([...detailBuildingTypes, remaining[0]]); }}
                    className="px-3 py-[5px] rounded-md border-[1.5px] border-dashed border-hm-blue bg-hm-blue-bg text-hm-blue-dark text-xs font-bold cursor-pointer font-[inherit] hover:brightness-95 transition-all">
                    + 유형 추가
                  </button>
                )}
              </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-2.5">
            ...
            <div>
              ...
              <div className="flex gap-2 py-1.5 items-center">
                ...
                <span className="text-xs text-hm-text-muted">대</span>
                <span className="text-xs text-[#3D4251] ml-1.5">건물주차총대수</span>
                <input type="number" min="0" value={bdParkingTotal} onChange={e => setBdParkingTotal(e.target.value)} placeholder="0" className={`${inputClassName} !w-11 !px-1.5 !py-1 !text-xs !text-center`} />
                <span className="text-xs text-hm-text-muted">대</span>
              </div>
            </div>
          </div>
      ...
      <Card className="mb-4">
        <div className={`flex justify-between items-center ${sec2Open ? 'mb-3' : ''}`}>
          <div onClick={() => setSec2Open(!sec2Open)} className="cursor-pointer flex-1">
            <div className="text-base font-bold text-hm-text">👤 건물주 정보</div>
            <div className="text-xs text-hm-text-muted mt-0.5">{`건물주 ${bdOwners.length}명 · 최대 4명`}</div>
          </div>
          <div className="flex items-center gap-2">
            {sec2Open && (sec2Edit ? (
              <>
              ...
              return (
                <div key={oi} className={`px-3 py-2.5 rounded-lg ${oi < bdOwners.length - 1 ? 'mb-2.5' : ''}`} style={{ background: c.bg, border: `1px solid ${c.border}` }}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold" style={{ color: c.color }}>건물주 {oi + 1}{c.label ? ` (${c.label})` : ""}</span>
                      {oi === 0 && bdOwners.length < 4 && (
                        <button onClick={addBdOwner}
                          className="px-2.5 py-0.5 rounded-[5px] border-[1.5px] border-dashed bg-transparent text-xs font-bold cursor-pointer font-[inherit] hover:brightness-90 transition-all" style={{ borderColor: c.color, color: c.color }}>+ 건물주 추가</button>
                      )}
                    </div>
                    {oi > 0 && (
                      <button onClick={() => removeBdOwner(oi)}
                        className="px-2 py-0.5 rounded-[5px] border border-red-300 bg-hm-danger-bg text-hm-danger text-xs font-bold cursor-pointer font-[inherit] hover:brightness-90 transition-all">✕ 삭제</button>
                    )}
                  </div>
