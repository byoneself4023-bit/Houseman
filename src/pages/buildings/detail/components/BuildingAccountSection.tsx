    ...
    <Card className="mb-4">
      <div className={`flex justify-between items-center ${secAcctOpen ? 'mb-3' : ''}`}>
        <div onClick={() => setSecAcctOpen(!secAcctOpen)} className="cursor-pointer flex-1">
          <div className="text-[15px] font-extrabold text-hm-text">🏦 건물 계좌 정보 & 계약 문자</div>
          <div className="text-[11px] text-hm-text-muted mt-0.5">계좌: 건물유형({detailBuildingTypes.join(" + ")})에서 자동 반영 · 계약 문자: 계약 시 발송할 문구</div>
        </div>
        <div className="flex items-center gap-2">
          {secAcctOpen && (secAcctEdit ? (
            <>
              <button onClick={() => setSecAcctEdit(false)} className="px-3.5 py-[5px] rounded-md border-[1.5px] border-hm-input-border bg-white text-hm-text-sub font-bold text-[11px] cursor-pointer font-[inherit] hover:bg-hm-bg-hover transition-colors">취소</button>
              <button onClick={() => setSecAcctEdit(false)} className="px-3.5 py-[5px] rounded-md border-none bg-hm-blue-dark text-white font-bold text-[11px] cursor-pointer font-[inherit] hover:brightness-90 transition-all">💾 저장</button>
            </>
          ) : (
            <button onClick={() => setSecAcctEdit(true)} className="px-3.5 py-[5px] rounded-md border-none bg-hm-blue-dark text-white font-bold text-[11px] cursor-pointer font-[inherit] hover:brightness-90 transition-all">✏️ 수정</button>
          ))}
      ...
      {secAcctOpen && <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-4`}>
        {/* Left: Account info */}
        <div>
          <div className="text-[13px] font-extrabold text-hm-blue-dark mb-2.5 px-2.5 py-1.5 bg-hm-blue-bg rounded-md border border-blue-200">🏦 건물 계좌 정보</div>
              ...
              {detailAcctTypes.map((aType, ai) => {
                ...
                return (
                  <div key={aType} className="px-3 py-2.5 rounded-lg" style={{ background: acctTypeBg[aType], border: `1.5px solid ${acctTypeColor[aType]}40` }}>
                    <div className="text-xs font-extrabold mb-2" style={{ color: acctTypeColor[aType] }}>{aType}</div>
                    <div className={`flex gap-1 flex-wrap ${curMode ? 'mb-2' : ''}`}>
                      {curOptions.map((opt: any) => (
                        <button key={opt.id} onClick={() => updateBldgAcct({ [modeKey]: opt.id, [ownerKey]: {} })}
                          className={`px-2.5 py-[5px] rounded-md text-[10px] font-bold cursor-pointer font-[inherit] transition-colors ${curMode === opt.id ? 'border-[1.5px] border-amber-400 bg-amber-50 text-amber-800' : 'border border-hm-input-border bg-white text-hm-text-sub hover:bg-hm-bg-hover'}`}
                          title={opt.desc}>
                          {opt.label}
                        </button>
                      ))}
                    ...
                    {curMode && (() => {
                      const hmSection = curHmUsage && (
                        <div key="hm" className="px-2.5 py-2 bg-[#F0F4FF] rounded-md border border-blue-200">
                          <div className="text-[9px] font-bold text-hm-blue-dark mb-1">🏗️ 하우스맨 계좌 <span className="text-hm-text-muted font-medium">({curHmUsage})</span></div>
                      ...
                      return (
                        <div className="flex flex-col gap-1.5">
                          ...
                          <div className="text-[10px] text-hm-text-sub py-1">💡 {flowMap[curMode]}</div>
                          ...
                          {aType === "단기" && curMode === "houseman" && (
                            <div className="mt-1.5 px-2.5 py-2 bg-white rounded-md border border-gray-200">
                              <div className="text-[9px] font-bold text-gray-700 mb-1.5">예치금 보관</div>
                                ...
                                {[{ id: "hm", label: "하우스맨 보관" }, { id: "owner", label: "건물주 보관" }].map(opt => {
                                  ...
                                  return (
                                    <button key={opt.id} onClick={() => updateBldgAcct({ depositHolder: opt.id })}
                                      className={`px-3 py-[5px] rounded-md text-[10px] font-bold cursor-pointer font-[inherit] transition-colors ${cur === opt.id ? 'border-[1.5px] border-hm-blue bg-hm-blue-bg text-hm-blue-dark' : 'border border-hm-input-border bg-white text-hm-text-sub hover:bg-hm-bg-hover'}`}>
                                      {opt.label}
                                    </button>
                                  );
                                })}
            ...
            return (
              <div className="mt-3.5 px-3.5 py-3 bg-amber-50 rounded-lg border-[1.5px] border-amber-200">
                <div className="flex justify-between items-center mb-2">
                  <div className="text-xs font-extrabold text-amber-800">🧮 입주금 계산식</div>
                  {calcTypes.length > 1 && (
                    <div className="flex gap-1">
                      {calcTypes.map((ct: string) => (
                        <button key={ct} onClick={() => updateBD({ calcType: ct })}
                          className={`px-2.5 py-[3px] rounded-[5px] text-[10px] font-bold cursor-pointer font-[inherit] transition-colors ${calcType === ct ? 'border-[1.5px] border-amber-400 bg-amber-50 text-amber-800' : 'border border-hm-input-border bg-white text-hm-text-muted hover:bg-hm-bg-hover'}`}>
                          {ct}
                        </button>
                      ))}
                    </div>
                ...
                {calcType === "단기" ? (
                  isSingleAcct || !curMode ? (
                    <div className="text-xs font-bold text-hm-text leading-[1.8]">
                      <span className="text-hm-danger font-extrabold">입주금 총합</span> = 보증금(예치금) + 월세(임대료) + 관리비 + 수도 + 인터넷TV
                    </div>
                  ) : (
                    <div className="text-[11px] text-hm-text-muted py-2">
        ...
        <div>
          <div className="text-[13px] font-extrabold text-hm-success mb-2.5 px-2.5 py-1.5 bg-green-50 rounded-md border border-green-200">📩 계약 문자</div>
          <div className="text-[10px] text-hm-text-muted mb-2">계약 체결 시 임차인에게 발송할 안내 문자를 입력하세요.</div>
          <textarea
            value={contractMsg}
          ...
          <div className="flex justify-end gap-1.5 mt-2">
            <button onClick={() => {
              ...
              className="px-3.5 py-1.5 rounded-md border border-green-200 bg-green-50 text-hm-success text-[11px] font-bold cursor-pointer font-[inherit] hover:brightness-95 transition-all">
              📋 복사
            </button>
          </div>
        </div>
