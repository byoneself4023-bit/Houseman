      ...
      <Card className="mb-4">
        <div className={`flex justify-between items-center ${sec3Open ? 'mb-3' : ''}`}>
          <div onClick={() => setSec3Open(!sec3Open)} className="cursor-pointer flex-1">
            <div className="text-base font-bold text-hm-text">🏢 담당자 & 계약 조건</div>
            <div className="text-xs text-hm-text-muted mt-0.5">역할별 담당자 · 수수료 · 정산계좌 · 이메일</div>
          </div>
        ...
        {sec3Open && <div className={`transition-opacity duration-200 ${sec3Edit ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-70'}`}>
          ...
          <div className="px-3 py-2 bg-[#F0F4FF] rounded-lg mb-3 border border-[#BFDBFE]">
            <div className="text-xs font-bold text-hm-blue-dark mb-1.5">👤 담당자 배정</div>
            <div className={`grid gap-1.5 ${isMobile ? 'grid-cols-2' : 'grid-cols-5'}`}>
              {staffRoles.map((d: any, i: number) => (
                <div key={i}>
                  ...
                  }} className={`${inputClassName} !px-2 !py-[5px] !text-xs cursor-pointer`}>
          ...
          <div className="mb-2.5">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-hm-text-muted">정산일 ({bdSettlementDates.length}회/월)</span>
              {bdSettlementDates.length < 3 && (
                <button onClick={addSettlementDate} className="text-xs text-hm-blue bg-transparent border-none cursor-pointer font-[inherit] font-bold hover:underline">+ 추가</button>
              )}
            </div>
            {detailBuildingTypes.includes("근생") && !detailBuildingTypes.includes("단기") && !detailBuildingTypes.includes("일반임대") ? (
              <div>
                <div className="flex gap-1.5 items-center mb-1">
                  <div className={`${inputClassName} !px-2 !py-1.5 !text-xs !bg-gray-100 !text-gray-500 flex-1`}>정산기간: 1일 ~ 말일 (고정)</div>
                </div>
                <div className="flex gap-1.5 items-center">
                  <span className="text-xs text-hm-text-muted min-w-[50px]">정산일</span>
                  <select value={bdSettlementDates[0] === "5" || bdSettlementDates[0] === (5 as any) ? "5" : "1"} onChange={e => setBdSettlementDates([e.target.value])} className={`${inputClassName} !px-2 !py-1.5 !text-xs cursor-pointer flex-1`}>
                    <option value="1">매월 1일</option>
                    <option value="5">매월 5일</option>
                  </select>
                  <span className="text-xs text-amber-500 font-semibold whitespace-nowrap">* 토/일/공휴일 → 다음 영업일</span>
                </div>
              </div>
            ) : (
              <div className="flex flex-col gap-1">
                {bdSettlementDates.map((d, i) => (
                  <div key={i} className="flex gap-1.5 items-center">
                    <span className="text-xs text-hm-text-muted min-w-[30px] font-semibold">{i + 1}차</span>
                    <select value={d} onChange={e => updateSettlementDate(i, e.target.value)} className={`${inputClassName} !px-2 !py-1.5 !text-xs cursor-pointer flex-1`}>
                      {[...Array(28).keys()].map(n => <option key={n + 1} value={String(n + 1)}>{n + 1}일</option>)}
                      <option value="말일">말일</option>
                    </select>
                    {bdSettlementDates.length > 1 && (
                      <button onClick={() => removeSettlementDate(i)} className="px-2 py-0.5 bg-transparent border border-hm-danger-border rounded text-hm-danger cursor-pointer text-xs font-[inherit] hover:bg-hm-danger-bg transition-colors">✕</button>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-2.5">
            <div>
              ...
              {detailBuildingTypes.includes("단기") && !detailBuildingTypes.includes("일반임대") && !detailBuildingTypes.includes("근생") ? (
                ...
                <select value={bdMgmtType} onChange={e => setBdMgmtType(e.target.value)} className={`${inputClassName} !px-2 !py-1.5 !text-xs cursor-pointer`}>
            ...
            <div className="flex items-center pt-3.5">
              <span className={`text-xs font-semibold ${(detailBuildingTypes.includes("단기") && !detailBuildingTypes.includes("일반임대") && !detailBuildingTypes.includes("근생")) ? 'text-gray-500' : bdMgmtType === "고정관리비" ? 'text-gray-500' : 'text-hm-blue-dark'}`}>
            ...
            <div>
              ...
              {emails.map((e, i) => (
                <div key={i} className={`flex gap-1 ${i < emails.length - 1 ? 'mb-[3px]' : ''}`}>
                  <input value={e} onChange={ev => { const u = [...emails]; u[i] = ev.target.value; setEmails(u); }} placeholder="example@email.com" className={`${inputClassName} !px-2 !py-[5px] !text-xs flex-1`} />
                  {emails.length > 1 && <button onClick={() => setEmails(emails.filter((_, j) => j !== i))} className="px-1.5 bg-transparent border-none text-hm-danger cursor-pointer text-sm hover:opacity-70 transition-opacity">✕</button>}
                </div>
              ))}
            </div>
          </div>
      ...
      <Card className="mb-4">
        <div className={`flex justify-between items-center ${sec4Open ? 'mb-3' : ''}`}>
          <div onClick={() => setSec4Open(!sec4Open)} className="cursor-pointer flex-1">
            <div className="text-base font-bold text-hm-text">🔧 협력업체</div>
            <div className="text-xs text-hm-text-muted mt-0.5">소방 · 승강기 · 청소 · 소독 등 외주업체</div>
          </div>
          <div className="flex items-center gap-2">
            {sec4Open && (sec4Edit ? (
              <>
          ...
          {[
            ...
            <div key={v.key} className={`px-3 py-2.5 rounded-lg border border-hm-border ${i % 2 === 0 ? 'bg-hm-bg-hover' : 'bg-white'}`}>
                ...
                <div className="flex gap-1">
                  {[{ id: "direct", label: "직접관리" }, { id: "vendor", label: "협력업체관리" }].map(m => (
                    <button key={m.id} onClick={() => setFireMode(m.id)}
                      className={`px-3 py-1 rounded-md text-xs font-bold cursor-pointer font-[inherit] transition-colors ${fireMode === m.id ? 'border-[1.5px] border-hm-danger bg-hm-danger-bg text-hm-danger' : 'border border-hm-input-border bg-white text-hm-text-muted'}`}>
                      {m.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-[80px_1fr] gap-2 items-center mb-1.5">
                <span className="text-xs text-hm-danger font-semibold">사용승인일</span>
                <div><input type="date" value={bdApprovalDate} onChange={e => setBdApprovalDate(e.target.value)} className={`${inputClassName} !px-2 !py-[5px] !text-xs max-w-[180px]`} /></div>
              </div>
              {fireMode === "vendor" && (
                <div className="grid grid-cols-[80px_1fr_1fr_1fr_1fr_auto] gap-2 items-center mb-1.5">
                  <span className="text-xs text-hm-text-muted font-semibold">협력업체</span>
                  <div><input value={bdVendors.fire.company} onChange={e => setBdVendor("fire", "company", e.target.value)} placeholder="업체명" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                  <div><input value={bdVendors.fire.phone} onChange={e => setBdVendor("fire", "phone", e.target.value)} placeholder="업체 연락처" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                  <div><input value={bdVendors.fire.contact} onChange={e => setBdVendor("fire", "contact", e.target.value)} placeholder="업체 담당자" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                  <div><input value={bdVendors.fire.contactPhone} onChange={e => setBdVendor("fire", "contactPhone", e.target.value)} placeholder="담당자 휴대폰" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                  <button className="px-2.5 py-1 rounded-md border border-hm-input-border bg-white text-xs cursor-pointer font-[inherit] text-hm-blue font-bold whitespace-nowrap hover:bg-hm-blue-bg transition-colors">📎 계약서</button>
                </div>
              )}
              <div className="grid grid-cols-[80px_1fr_1fr_1fr] gap-2 items-center pt-1.5" style={{ borderTop: `1px dashed ${v.color}40` }}>
                <span className="text-xs font-bold" style={{ color: v.color }}>👤 소방안전관리자</span>
                <div><input value={bdVendors.fire.manager} onChange={e => setBdVendor("fire", "manager", e.target.value)} placeholder="성명" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors.fire.managerPhone} onChange={e => setBdVendor("fire", "managerPhone", e.target.value)} placeholder="연락처" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors.fire.managerNote} onChange={e => setBdVendor("fire", "managerNote", e.target.value)} placeholder="자격/비고" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
              </div>
            </div>
          ) : v.type === "withManager" ? (
            <div key={v.key} className={`px-3 py-2.5 rounded-lg border border-hm-border ${i % 2 === 0 ? 'bg-hm-bg-hover' : 'bg-white'}`}>
              ...
              <div className="grid grid-cols-[80px_1fr_1fr_1fr_1fr_auto] gap-2 items-center mb-1.5">
                <span className="text-xs text-hm-text-muted font-semibold">협력업체</span>
                <div><input value={bdVendors[v.key]?.company || ""} onChange={e => setBdVendor(v.key, "company", e.target.value)} placeholder="업체명" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors[v.key]?.phone || ""} onChange={e => setBdVendor(v.key, "phone", e.target.value)} placeholder="업체 연락처" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors[v.key]?.contact || ""} onChange={e => setBdVendor(v.key, "contact", e.target.value)} placeholder="업체 담당자" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors[v.key]?.contactPhone || ""} onChange={e => setBdVendor(v.key, "contactPhone", e.target.value)} placeholder="담당자 휴대폰" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <button className="px-2.5 py-1 rounded-md border border-hm-input-border bg-white text-xs cursor-pointer font-[inherit] text-hm-blue font-bold whitespace-nowrap hover:bg-hm-blue-bg transition-colors">📎 계약서</button>
              </div>
              <div className="grid grid-cols-[80px_1fr_1fr_1fr] gap-2 items-center pt-1.5" style={{ borderTop: `1px dashed ${v.color}40` }}>
                <span className="text-xs font-bold" style={{ color: v.color }}>👤 {v.person}</span>
                <div><input value={bdVendors[v.key]?.manager || ""} onChange={e => setBdVendor(v.key, "manager", e.target.value)} placeholder="성명" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors[v.key]?.managerPhone || ""} onChange={e => setBdVendor(v.key, "managerPhone", e.target.value)} placeholder="연락처" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors[v.key]?.managerNote || ""} onChange={e => setBdVendor(v.key, "managerNote", e.target.value)} placeholder="자격/비고" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
              </div>
            </div>
          ) : (
              ...
              <div className="grid grid-cols-[160px_1fr_1fr_1fr_1fr_auto] gap-2 items-center">
                {v.label.startsWith("custom") ? (
                  <div className="flex items-center gap-1.5">
                    <span>{v.icon}</span>
                    <input value={bdVendors[v.key]?.label || ""} onChange={e => setBdVendor(v.key, "label", e.target.value)} placeholder={`기타${v.label.slice(-1)} (입력)`} className={`${inputClassName} !px-2 !py-[5px] !text-xs !font-bold`} />
                  </div>
                ) : (
                  <div className="text-xs font-bold flex items-center gap-1.5" style={{ color: v.color }}>
                    <span>{v.icon}</span> {v.label}
                ...
                <div><input value={bdVendors[v.key]?.company || ""} onChange={e => setBdVendor(v.key, "company", e.target.value)} placeholder="회사명" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors[v.key]?.phone || ""} onChange={e => setBdVendor(v.key, "phone", e.target.value)} placeholder="연락처" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors[v.key]?.contact || ""} onChange={e => setBdVendor(v.key, "contact", e.target.value)} placeholder="담당자명" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <div><input value={bdVendors[v.key]?.contactPhone || ""} onChange={e => setBdVendor(v.key, "contactPhone", e.target.value)} placeholder="담당자 휴대폰" className={`${inputClassName} !px-2 !py-[5px] !text-xs`} /></div>
                <button className="px-2.5 py-1 rounded-md border border-hm-input-border bg-white text-xs cursor-pointer font-[inherit] text-hm-blue font-bold whitespace-nowrap hover:bg-hm-blue-bg transition-colors">📎 계약서</button>
              </div>
            </div>
      ...
      <Card className="mb-4">
        <div className={`flex justify-between items-center ${sec5Open ? 'mb-3' : ''}`}>
          <div onClick={() => setSec5Open(!sec5Open)} className="cursor-pointer flex-1">
            <div className="text-base font-bold text-hm-text">📝 건물 특이사항</div>
            <div className="text-xs text-hm-text-muted mt-0.5">관리 참고사항 · 이력 기록</div>
          </div>
          <div className="flex items-center gap-2">
            {sec5Open && (notesEdit ? (
              <>
      ...
      <Card className="mb-4">
        <div className={`flex justify-between items-center ${sec6Open ? 'mb-3' : ''}`}>
          <div onClick={() => setSec6Open(!sec6Open)} className="cursor-pointer flex-1">
            <div className="text-base font-bold text-hm-text">✅ 시설 체크리스트</div>
            <div className="text-xs text-hm-text-muted mt-0.5">순회 시 점검할 시설 항목 관리 · {bdFacilityChecklist.length}개 항목</div>
          </div>
          <div className="flex items-center gap-2">
            ...
            {bdFacilityChecklist.map((item, i) => (
              <div key={i} className="flex items-center justify-between px-3 py-2 bg-hm-bg-slate rounded-lg border border-hm-border">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-hm-text">{item}</span>
                </div>
