          ...
          <>
            <Card className="mb-4">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="text-xl font-bold text-hm-text">{r.name}</div>
              ...
              {/* 계약서 파일 */}
              <div className="text-xs font-bold text-hm-text mb-2 pb-1.5 border-b-[1.5px] border-hm-border">📎 계약서</div>
              ...
              {(r.moveInPhotos || []).length > 0 && (
                <div className="mt-4">
                  <div className="text-xs font-bold text-hm-success mb-2 pb-1.5 border-b-2 border-[#D1FAE5]">🏠 입주사진 ({r.moveInPhotos!.length}장) <span className="text-xs text-hm-text-muted font-semibold">{r.moveIn || ""}</span></div>
              ...
              {((r.moveInCheckPhotos || []).length > 0 || (r.moveOutPhotos || []).length > 0) && (
                <div className="mt-4">
                  <div className="text-xs font-bold text-hm-text mb-2.5 pb-1.5 border-b-[1.5px] border-hm-border">📸 호실 상태 비교 (시작 → 끝)</div>
                  <div className="grid grid-cols-2 gap-3">
                    {/* 시작: 입주체크사진 */}
                    <div className="border-2 border-hm-warning-border rounded-xl p-3 bg-hm-warning-bg">
                      <div className="text-xs font-bold text-hm-warning mb-2">📋 입주체크사진 (시작) <span className="text-xs text-hm-text-muted font-semibold">{(r.moveInCheckPhotos || []).length}장</span></div>
                    ...
                    <div className="border-2 border-hm-danger-border rounded-xl p-3 bg-hm-danger-bg">
                      <div className="text-xs font-bold text-hm-danger mb-2">🚪 퇴실사진 (끝) <span className="text-xs text-hm-text-muted font-semibold">{(r.moveOutPhotos || []).length}장</span></div>
                      ...
                      return (
                        <div key={i} className="flex justify-between items-center px-3 py-2 bg-hm-bg-slate rounded-lg border border-hm-border">
                          <div>
                            ...
                            <span className="text-xs text-hm-text-sub ml-2">{rec.phone || ""}</span>
                            <div className="text-xs text-hm-text-muted mt-0.5">{rec.moveIn || "-"} ~ {rec.moveOut || "-"} · 월세 {fmt(rec.rent)}원</div>
                          </div>
                          <span className="text-xs font-bold px-2 py-0.5 rounded" style={{ background: rcs.bg, color: rcs.c }}>{rec.reason}</span>
                        </div>
                      );
                    })}
                  </div>
          ...
          <>
            ...
            <Card ref={settlementRef} className="max-w-[720px]">
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                <div className={`px-[18px] py-2 rounded-lg text-sm font-bold border-[1.5px] ${r.reason === "만기전퇴실" ? 'bg-hm-danger-bg text-hm-danger border-hm-danger-border' : 'bg-[#F0FDF4] text-hm-success border-hm-success-border'}`}>
                  {r.reason}
                </div>
              </div>
                ...
                <div className="grid grid-cols-[3fr_2fr] gap-4">
                  ...
                  <div>
                    ...
                    {(() => {
                      ...
                      return (<>
                        <div className="text-xs font-bold text-hm-warning mb-2 pb-1.5 border-b-[1.5px] border-hm-warning-border">✏️ 수기 입력 내역</div>
                        {(r.manElec||[]).map((row: any,i: number) => pn(row.amt) > 0 && (
                          <div key={`e${i}`} className="flex justify-between py-1 border-b border-hm-danger-bg">
                            <span className="text-xs text-hm-warning">전기{row.period ? ` (${row.period})` : ` ${i+1}`}</span>
                        ...
                        {(r.manGas||[]).map((row: any,i: number) => pn(row.amt) > 0 && (
                          <div key={`g${i}`} className="flex justify-between py-1 border-b border-hm-danger-bg">
                            <span className="text-xs text-hm-warning">가스{row.period ? ` (${row.period})` : ` ${i+1}`}</span>
                            <span className="text-xs font-semibold">{fmt(pn(row.amt))}</span>
                          </div>
                        ))}
                        {(r.manRepair ?? 0) > 0 && (
                          <div className="flex justify-between py-1 border-b border-hm-danger-bg">
                            <span className="text-xs text-hm-warning">수리비{r.manRepairDesc ? ` (${r.manRepairDesc})` : ""}</span>
                            <span className="text-xs font-semibold">{fmt(r.manRepair!)}</span>
                          </div>
                        )}
                        {(r.manWaste ?? 0) > 0 && (
                          <div className="flex justify-between py-1 border-b border-hm-danger-bg">
                            <span className="text-xs text-hm-warning">폐기물{r.manWasteDesc ? ` (${r.manWasteDesc})` : ""}</span>
                        ...
                        {(r.manOther||[]).map((row: any,i: number) => pn(row.amt) > 0 && (
                          <div key={`o${i}`} className="flex justify-between py-1 border-b border-hm-danger-bg">
                            <span className="text-xs text-hm-warning">기타{i+1}{row.desc ? ` (${row.desc})` : ""}</span>
                            <span className="text-xs font-semibold">{fmt(pn(row.amt))}</span>
                          </div>
                        ))}
                        {(r.manRestoration ?? 0) > 0 && (
                          <div className="flex justify-between py-1 border-b border-hm-danger-bg">
                            <span className="text-xs text-[#7C3AED]">원상복구비{r.manRestorationDesc ? ` (${r.manRestorationDesc})` : ""}</span>
                  ...
                  {/* Right - 정산 계산 */}
                  <div>
                    <div className="text-xs font-bold text-hm-text mb-2 pb-1.5 border-b-[1.5px] border-hm-border">정산 내역</div>
                    ...
                    <div className="bg-[#F0FDF4] rounded-[10px] px-3.5 py-2.5 mb-2.5 border border-hm-success-border">
                      ...
                      <div className="flex justify-between py-1.5 mt-1 border-t-[1.5px] border-hm-success-border">
                        ...
                        <span className="text-sm font-bold text-hm-success">{fmt(r.deposit + (r.totalRefund || 0))}</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-hm-danger-bg rounded-[10px] px-3.5 py-2.5 mb-2.5 border border-hm-danger-border">
                      ...
                      <div className="flex justify-between py-1.5 mt-1 border-t-[1.5px] border-hm-danger-border">
                        ...
                        <span className="text-sm font-bold text-hm-danger">-{fmt(r.totalDeduct || 0)}</span>
                      </div>
                    </div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {reasons.map(r => (
          <button key={r} onClick={() => setFilterReason(r)}
            ...
            {r} <span className="font-bold">{reasonCounts[r]}</span>
          </button>
        ))}
      </div>
            ...
            <thead>
              <tr className="border-b-2 border-hm-border">
                {["건물명","호실","입주자","연락처","퇴실일","정산금액","사유"].map((h, i) => (
                  <th key={i} className={`px-2 py-2.5 text-xs font-bold text-hm-text-muted whitespace-nowrap ${i === 5 ? 'text-right' : 'text-left'}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((r, i) => {
                ...
                return (
                  <tr key={i} onClick={() => { setSelected(r); setViewMode("info"); }}
                    ...
                    <td className="px-2 py-2.5 text-xs text-hm-text-sub">{r.phone || "-"}</td>
                    <td className="px-2 py-2.5 text-xs">{r.moveOut ? r.moveOut.slice(2) : "-"}</td>
                    ...
                    <td className="px-2 py-2.5">
                      <span className="text-xs font-bold px-2 py-[3px] rounded-[5px] whitespace-nowrap" style={{ background: rc.bg, color: rc.c }}>{r.reason}</span>
                    </td>
                  </tr>
                );
              })}
