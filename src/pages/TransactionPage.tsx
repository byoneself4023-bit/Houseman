  ...
  return (
    <div>
      ...
      <Card className="mb-4 !px-[18px] !py-3.5 bg-hm-bg-slate border border-hm-border">
        <div className="text-xs font-bold text-hm-text mb-2">a.bankda.com 연동 자동 입금 확인</div>
        ...
        <div className="mt-2 px-3 py-2.5 rounded-lg bg-hm-blue-bg border border-[#BFDBFE]">
          <div className="text-xs font-bold text-hm-blue-dark mb-1.5">자동 확인 규칙 (100% 자동매칭)</div>
          <div className="text-xs text-[#1E40AF] leading-8">
        ...
        <div className="mt-1.5 text-xs text-hm-text-muted leading-relaxed">
      ...
      <div className={`grid gap-3 mb-5 ${isMobile ? 'grid-cols-2' : 'grid-cols-4'}`}>
        <Card className="bg-hm-success-bg border border-[#BBF7D0]">
          <div className="text-xs text-hm-text-muted font-semibold mb-1">이번달 입금</div>
          <div className="text-xl font-bold text-hm-success">{fmt(transactions.filter(t => t.type === "입금").reduce((s, t) => s + t.amount, 0))}원</div>
        </Card>
        <Card className="bg-hm-danger-bg border border-hm-danger-border">
          <div className="text-xs text-hm-text-muted font-semibold mb-1">총 미수금</div>
          <div className="text-xl font-bold text-hm-danger">{fmt(Object.values(roomBalances).reduce((s, v) => s + v, 0))}원</div>
        </Card>
        <Card className="bg-hm-blue-bg border border-[#BFDBFE]">
          <div className="text-xs text-hm-text-muted font-semibold mb-1">미납 호실</div>
          <div className="text-xl font-bold text-hm-blue">{overdueRooms.length}건</div>
        </Card>
        <Card className="bg-[#FDF4FF] border border-[#E9D5FF] cursor-pointer hover:shadow-md transition-shadow" onClick={() => setShowBankda(!showBankda)}>
          <div className="text-xs text-hm-text-muted font-semibold mb-1">뱅크다 미매칭</div>
          <div className="text-xl font-bold text-[#7C3AED]">{bankMatches.length}건</div>
        </Card>
      </div>
            ...
            <Card className={`mb-4 cursor-pointer ${totalDebtAmt > 0 ? 'border-2 border-hm-danger bg-hm-danger-bg' : 'border-[1.5px] border-hm-border bg-hm-bg-slate'}`} onClick={() => setShowDebtSection(!showDebtSection)}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className={`text-sm font-bold ${totalDebtAmt > 0 ? 'text-hm-danger' : 'text-hm-text'}`}>퇴실 미수금</div>
                    <div className="text-xs text-hm-text-muted">퇴실 정산 후 미회수 잔액</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className={`text-lg font-bold ${totalDebtAmt > 0 ? 'text-hm-danger' : 'text-hm-success'}`}>{totalDebtAmt > 0 ? fmt(totalDebtAmt) + "원" : "없음"}</div>
                  <div className="text-xs text-hm-text-muted">미수 {pendingDebts.length}건 / 전체 {myDebts.length}건</div>
                </div>
              </div>
            </Card>
            {showDebtSection && (
              <Card className="mb-4 border-[1.5px] border-hm-danger-border bg-white">
                <div className="text-sm font-bold text-hm-danger mb-3">🚪 퇴실 미수금 내역</div>
                <div className="flex gap-1.5 mb-3">
                  {["전체", "미수", "회수완료", "포기"].map(s => {
                    const cnt = s === "전체" ? myDebts.length : myDebts.filter(d => d.status === s).length;
                    return (
                      <span key={s} className={`text-xs font-bold px-2.5 py-1 rounded-md ${s === "미수" ? 'bg-hm-danger-bg text-hm-danger' : s === "회수완료" ? 'bg-hm-success-bg text-hm-success' : s === "포기" ? 'bg-[#F3F4F6] text-hm-text-muted' : 'bg-hm-blue-bg text-hm-blue-dark'}`}>
                        {s} {cnt}
                      </span>
                    );
                  })}
                ...
                {myDebts.length === 0 ? (
                  <div className="text-center py-5 text-[#B0B5C1] text-sm">퇴실 미수금 내역이 없습니다</div>
                ) : (
                    ...
                    {myDebts.map((d, i) => (
                      <div key={d.id || i} className={`flex items-center justify-between px-3 py-2.5 rounded-lg ${d.status === "미수" ? 'bg-hm-danger-bg border border-hm-danger-border' : d.status === "회수완료" ? 'bg-hm-success-bg border border-[#BBF7D0]' : 'bg-hm-bg-slate border border-hm-border'}`}>
                        <div>
                          ...
                          <div className="text-xs text-hm-text-muted">퇴실일: {d.date}</div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`text-base font-bold ${d.status === "미수" ? 'text-hm-danger' : d.status === "회수완료" ? 'text-hm-success' : 'text-hm-text-muted'}`}>{fmt(d.amount)}원</span>
                          {d.status === "미수" && (
                            <div className="flex gap-1">
                              <button onClick={(e) => { e.stopPropagation(); setMoveoutDebts(prev => prev.map(x => x.id === d.id ? { ...x, status: "회수완료" } : x)); }}
                                className="text-xs font-bold px-2 py-1 rounded-[5px] border-none bg-hm-success text-white cursor-pointer font-[inherit] hover:opacity-90 active:scale-95 transition-all">
                                회수완료
                              </button>
                              <button onClick={(e) => { e.stopPropagation(); if (confirm("정말 포기하시겠습니까?")) setMoveoutDebts(prev => prev.map(x => x.id === d.id ? { ...x, status: "포기" } : x)); }}
                                className="text-xs font-bold px-2 py-1 rounded-[5px] border border-hm-input-border bg-white text-hm-text-muted cursor-pointer font-[inherit] hover:bg-hm-bg-hover active:scale-95 transition-all">
                                포기
                              </button>
                            </div>
                          )}
        ...
        <Card className="mb-4 border-2 border-[#7C3AED] bg-[#FEFBFF]">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-1.5">
              <span className="text-xs px-2.5 py-1 rounded-md bg-hm-success-bg text-hm-success font-bold">100% {auto100.length}건</span>
              <span className="text-xs px-2.5 py-1 rounded-md bg-[#FEF3C7] text-[#92400E] font-bold">수동확인 {manualNeeded.length}건</span>
              <button onClick={() => setShowDepositNames(!showDepositNames)}
                className={`text-xs px-2.5 py-1 rounded-md border font-bold cursor-pointer font-[inherit] transition-colors ${showDepositNames ? 'border-[#E9D5FF] bg-[#7C3AED] text-white' : 'border-[#E9D5FF] bg-[#F5F3FF] text-[#7C3AED] hover:bg-[#EDE9FE]'}`}>
                지정 입금자명
              </button>
            </div>
          </div>
            ...
            <div className="mb-4 px-3.5 py-3 rounded-[10px] bg-[#F5F3FF] border border-[#E9D5FF]">
              ...
              {Object.keys(depositNames).length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-2.5">
                  {Object.entries(depositNames).map(([key, name]) => (
                    <div key={key} className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-white border border-[#E9D5FF] text-xs">
                      ...
                      <span onClick={() => setDepositNames(prev => { const n = { ...prev }; delete n[key]; return n; })}
                        className="text-hm-danger cursor-pointer font-bold ml-1 hover:opacity-70 transition-opacity">×</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {Object.keys(nameAliases).length > 0 && (
                <div className="mb-2.5">
                  <div className="text-xs text-hm-text-muted mb-1">자동 저장된 입금자명</div>
                  <div className="flex flex-wrap gap-1">
                    {Object.entries(nameAliases).map(([key, names]) => names.map((n, i) => (
                      <span key={`${key}_${i}`} className="text-xs px-2 py-0.5 rounded bg-hm-success-bg text-hm-success font-semibold">
                        {key.replace("_", " ")} ← {n}
                      </span>
                    )))}
                  </div>
              ...
              <div className="flex gap-1.5 items-center">
                ...
                <input value={editDepVal} onChange={e => setEditDepVal(e.target.value)} placeholder="지정 입금자명"
                  className="px-2.5 py-1.5 rounded-md border-[1.5px] border-hm-input-border text-xs font-[inherit] w-[120px] outline-none focus:ring-2 focus:ring-ring transition-colors" />
                <button onClick={() => {
          ...
          {auto100.length > 0 && (
            <button onClick={() => auto100.forEach(m => handleMatch(m.bankTx, m.tenant!, m.saveName))}
              className="w-full py-2.5 rounded-lg border-none bg-hm-success text-white text-sm font-bold cursor-pointer font-[inherit] mb-3 hover:opacity-90 active:scale-[0.98] transition-all">
              ...
              return (
                <div key={bankTx.id} className={`px-3.5 py-3 rounded-[10px] border-[1.5px] ${score === 100 ? 'bg-[#F0FDF4] border-[#BBF7D0]' : score >= 50 ? 'bg-[#FFFBEB] border-[#FDE68A]' : 'bg-hm-danger-bg border-hm-danger-border'}`}>
                  ...
                  <div className="flex justify-between items-center mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-hm-text">{bankTx.depositor}</span>
                      <span className="text-xs text-hm-text-muted">{bankTx.date.slice(5)} · {bankTx.bank}은행</span>
                    </div>
                    <span className="text-sm font-bold text-hm-success">{fmt(bankTx.amount)}원</span>
                  </div>
                  ...
                  {tenant ? (
                    <div className="flex items-center justify-between flex-wrap gap-1.5">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className={`text-xs font-bold px-2.5 py-0.5 rounded-md text-white ${score === 100 ? 'bg-hm-success' : score >= 50 ? 'bg-[#F59E0B]' : 'bg-[#EF4444]'}`}>
                          {score}%
                        </span>
                        <span className="text-xs font-bold text-hm-text">
                          {tenant.building} {tenant.room}호 {tenant.name}
                        </span>
                        <span className="text-xs text-hm-text-muted">({reason})</span>
                        {saveName && <span className="text-xs px-1.5 py-0.5 rounded bg-hm-blue-bg text-hm-blue-dark font-semibold">이름 자동저장</span>}
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => handleMatch(bankTx, tenant, saveName)}
                          className="text-xs font-bold px-3 py-1 rounded-md border-none bg-hm-success text-white cursor-pointer font-[inherit] hover:opacity-90 active:scale-95 transition-all">
                          {score === 100 ? "확인" : "승인"}
                        </button>
                        {score < 100 && (
                          <button onClick={() => setManualAssign(isManual ? null : bankTx.id)}
                            className="text-xs font-bold px-3 py-1 rounded-md border border-hm-input-border bg-white text-hm-text-sub cursor-pointer font-[inherit] hover:bg-hm-bg-hover transition-colors">
                            {isManual ? "닫기" : "수동"}
                          </button>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs font-bold px-2.5 py-0.5 rounded-md bg-[#EF4444] text-white">0%</span>
                        <span className="text-xs text-hm-danger font-semibold">매칭 대상 없음</span>
                      </div>
                      <button onClick={() => setManualAssign(isManual ? null : bankTx.id)}
                        className="text-xs font-bold px-3 py-1 rounded-md border border-hm-input-border bg-white text-hm-text-sub cursor-pointer font-[inherit] hover:bg-hm-bg-hover transition-colors">
                        {isManual ? "닫기" : "수동 지정"}
                  ...
                  {isManual && (
                    <div className="mt-2.5 px-3 py-2.5 rounded-lg bg-hm-bg-slate border border-hm-border">
                      <div className="text-xs font-bold text-hm-text-sub mb-2">임차인 선택</div>
                      {candidates && candidates.length > 0 && (
                        <div className="mb-2">
                          <div className="text-xs text-hm-text-muted mb-1">추천 (유사도순)</div>
                          <div className="flex flex-wrap gap-1">
                            {candidates.slice(0, 5).map((c, ci) => (
                              <button key={ci} onClick={() => handleMatch(bankTx, c.tenant, true)}
                                className="text-xs font-semibold px-2.5 py-1 rounded-md border border-hm-input-border bg-white cursor-pointer font-[inherit] flex items-center gap-1 hover:bg-hm-bg-hover transition-colors">
                                <span className={`text-xs font-bold px-1.5 py-px rounded ${c.score >= 80 ? 'bg-[#D1FAE5] text-hm-success' : c.score >= 50 ? 'bg-[#FEF3C7] text-[#92400E]' : 'bg-[#FEE2E2] text-hm-danger'}`}>{c.score}%</span>
                      ...
                      <div className="text-xs text-hm-text-muted mb-1">전체 임차인</div>
                      <div className="max-h-[150px] overflow-y-auto flex flex-wrap gap-1">
                        {myTenants.map((t, ti) => (
                          <button key={ti} onClick={() => handleMatch(bankTx, t, true)}
                            className="text-xs px-2 py-0.5 rounded-[5px] border border-hm-input-border bg-white cursor-pointer font-[inherit] hover:bg-hm-bg-hover transition-colors">
        ...
        <Card className="mb-4 border-2 border-hm-success">
          {done ? (
            <div className="text-center p-5">
              ...
              <div className="text-base font-bold text-hm-success mt-2">입금 처리 완료</div>
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-3 gap-2.5 mb-2.5">
                ...
                <div>
                  <div className="text-xs text-hm-text-muted mb-1">호실</div>
                ...
                <div>
                  <div className="text-xs text-hm-text-muted mb-1">입금액</div>
              ...
              <div className="grid grid-cols-[120px_1fr_auto] gap-2.5 items-end">
                ...
                <button onClick={handleSubmit}
                  className="px-6 py-2 rounded-lg border-none bg-hm-success text-white text-sm font-bold cursor-pointer font-[inherit] whitespace-nowrap hover:opacity-90 active:scale-95 transition-all">등록</button>
              </div>
              {formBld && formRoom && (
      ...
      {isMobile ? (
        <div className="flex flex-col gap-1.5">
          {sortedTx.length === 0 && <div className="text-center py-10 text-[#B0B5C1] text-sm">입출금 내역이 없습니다</div>}
          {sortedTx.map((r, i) => (
            <Card key={i} className="!px-3 !py-2.5">
              <div className="flex justify-between items-start mb-1">
                <div className="flex items-center gap-1.5">
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${r.type === "입금" ? 'bg-hm-success-bg text-hm-success' : 'bg-hm-danger-bg text-hm-danger'}`}>{r.type}</span>
                  <span className="text-sm font-bold">{r.name}</span>
                </div>
                <span className={`text-sm font-bold ${r.type === "입금" ? 'text-hm-success' : 'text-hm-danger'}`}>{r.type === "입금" ? "+" : "-"}{fmt(r.amount)}원</span>
              </div>
              <div className="text-xs text-hm-text-sub">{r.building} {r.room}호 · {r.date.slice(5)} · {r.method}</div>
              {r.note && <div className="text-xs text-hm-text-muted mt-0.5">{r.note}</div>}
            </Card>
          ))}
        </div>
      ) : (
      <Table
        columns={[
          ...
          { label: "유형", render: (r: Record<string, any>) => <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${r.type === "입금" ? 'bg-hm-success-bg text-hm-success' : 'bg-hm-danger-bg text-hm-danger'}`}>{r.type}</span> },
          { label: "건물", render: (r: Record<string, any>) => <span className="font-semibold text-xs">{r.building}</span> },
          ...
          { label: "금액", align: "right" as const, render: (r: Record<string, any>) => <span className={`font-bold ${r.type === "입금" ? 'text-hm-success' : 'text-hm-danger'}`}>{r.type === "입금" ? "+" : "-"}{fmt(r.amount)}원</span> },
          { label: "방법", render: (r: Record<string, any>) => <span className="text-xs text-hm-text-muted">{r.method}</span> },
          { label: "잔액", align: "right" as const, render: (r: Record<string, any>) => {
            const bal = roomBalances[`${r.building}_${r.room}`] || 0;
            return <span className={`font-bold text-xs ${bal > 0 ? 'text-hm-danger' : 'text-hm-success'}`}>{fmt(bal)}원</span>;
          }},
          { label: "비고", render: (r: Record<string, any>) => <span className="text-xs text-hm-text-muted">{r.note}</span> },
        ]}
        data={sortedTx}
      />
      )}
