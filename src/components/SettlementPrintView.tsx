...
// TODO: Phase 1에서 정확한 타입으로 교체
type SettlementData = any;

interface SettlementPrintViewProps {
  /** 정산 데이터 (bs) — 구조가 매우 복잡하여 Phase 1에서 정확한 타입으로 교체 예정 */
  data: SettlementData; // TODO: Phase 1에서 정확한 타입으로 교체
  onClose: () => void;
}

const fmt = (n: number | null | undefined): string => n?.toLocaleString('ko-KR') ?? '0';

const toKoreanAmount = (n: number | null | undefined): string => {
  if (!n || n === 0) return '영원';
...
const buildCoverItems = (bs: SettlementData): CoverItem[] => {
  const { cfg } = bs;
...
// 메인 컴포넌트
// ============================================================
export const SettlementPrintView: React.FC<SettlementPrintViewProps> = ({ data, onClose }) => {
  const bs = data;
  ...
  const coverTotal = coverItems.reduce((s: number, item: CoverItem) => s + item.amount, 0);
  ...
  const refDeposit = bs.roomSettlements.reduce((s: number, r: any) => s + (r.deposit || 0), 0);
  const refRent = bs.roomSettlements.reduce((s: number, r: any) => s + (r.rent || 0), 0);
  const refMgmt = bs.roomSettlements.reduce((s: number, r: any) => s + (r.mgmt || 0), 0);
    ...
    <div className="stl-print-overlay">
      ...
      <div className="stl-toolbar">
        <div className="stl-toolbar-left">
          <button className="stl-btn stl-btn-back" onClick={onClose}>
            ← 돌아가기
          </button>
          <span style={{ fontSize: 14, fontWeight: 600 }}>{bs.building} 정산서 미리보기</span>
        </div>
        <div className="stl-toolbar-right">
          <button className="stl-btn stl-btn-print" onClick={() => window.print()}>
            인쇄
          </button>
        </div>
      </div>
      ...
      <div className="stl-pages">
        ...
        <div className="stl-page">
          ...
          <div className="stl-info">
            ...
            <div className="stl-info-item">
              ...
              <span className="stl-info-value">
                {bs.tenantCount}세대 (신규 {bs.moveInCount}, 퇴실 {bs.moveOutCount})
              </span>
            </div>
          </div>
          ...
          <div className="stl-final">
            ...
            <div className="stl-final-amt">
              {bs.finalAmount < 0 ? '-' : ''}
              {fmt(Math.abs(bs.finalAmount))}원
            </div>
          ...
          <div className="stl-ref">
            <div
              style={{ fontWeight: 700, marginBottom: '1.5mm', fontSize: '9pt', color: '#374151' }}
            >
              참고 합계
            </div>
        ...
        <div className="stl-page">
          <div style={{ textAlign: 'center', marginBottom: '6mm' }}>
            <div
              style={{ fontSize: '15pt', fontWeight: 800, color: '#111', marginBottom: '1.5mm' }}
            >
              {bs.building} 정산 상세내역
            </div>
            ...
            <tbody>
              {bs.roomSettlements.map((r: any, i: number) => (
                <tr key={i}>
                  ...
                  <td>{r.name}</td>
                  <td className="c" style={{ fontSize: '7.5pt' }}>
                    {r.moveIn?.slice(2)}
                  </td>
          ...
          {bs.moveOutSettlements.length > 0 && (
            <>
              <div className="stl-sec-title">
                {bs.moveInSettlements?.length > 0 ? '3' : '2'}. 퇴실 정산 (
                {bs.moveOutSettlements.length}건)
              </div>
              {bs.moveOutSettlements.map((mt: any, i: number) => (
                <div key={i} style={{ marginBottom: '4mm' }}>
                  ...
                  <table className="stl-dtl-table">
                    <tbody>
                      <tr>
                        <td style={{ width: '20%', background: '#f5f5f5', fontWeight: 600 }}>
                          퇴실일
                        </td>
                      ...
                      <tr style={{ borderTop: '2px solid #333' }}>
                        ...
                        <td style={{ background: '#e8f0fe', fontWeight: 800, fontSize: '10pt' }}>
                          최종 환불
                        </td>
                        <td
                          className="r b"
          ...
          {cfg.vat && bs.vatInfo.tax > 0 && (
            <>
              <div className="stl-sec-title" style={{ marginTop: '5mm' }}>
                부가가치세 계산
              </div>
          ...
          {/* 미납 */}
          {bs.roomSettlements.some((r: any) => (r.delinquent || 0) > 0) && (
            <div className="stl-delinq">
              미납 안내:{' '}
              {bs.roomSettlements
                .filter((r: any) => (r.delinquent || 0) > 0)
                .map((r: any) => `${r.room}호 ${r.name} ${fmt(r.delinquent)}원`)
                .join(', ')}
            </div>
          )}
