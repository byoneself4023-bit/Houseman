...
- 단위 테스트: {파일} 어떤 케이스
- 통합 테스트: {파일} 회귀
- 회귀: BE 117 @Test 중 영향 받는 N건

### §7 운영자 결정 라운드 (§3 옵션 N건 잠금)

**implementer 진입 전 운영자가 잠가야 할 결정 영역**. §3 옵션 ≥2개 중 채택 명시 + 응답 양식 제공.
...
**원칙**:
- §3에서 도출된 모든 옵션 영역을 §7에 1:1 매핑 (누락 금지)
- 사용자 입력 필요 영역 (예: Phase 7~12 슬롯) = "사용자 결정 잠금 요청" 명시
- 잠금 후 implementer delegation prompt에 결정 결과 전달 (subagent history 미공유 대응 `docs/Subagents.md §[5]`)
```
