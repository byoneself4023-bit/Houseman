    ...
    fun createExpense() {
        ...
        mockMvc.perform(
            post("/api/settlements/expenses")
                ...
                .content("""{"buildingId": 2, "month": "2026-04", "category": "repair", "description": "테스트 수선비", "amount": 100000, "date": "2026-04-01"}""")
        )
    ...
    fun deleteExpense() {
        ...
        val result = mockMvc.perform(
            post("/api/settlements/expenses")
                .header("Authorization", "Bearer $token")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"buildingId": 2, "month": "2026-04", "category": "other", "description": "삭제대상", "amount": 50000, "date": "2026-04-15"}""")
        )
            .andExpect(status().isCreated)
            .andReturn()
    ...
    fun calculateSettlement() {
        ...
        mockMvc.perform(
            post("/api/settlements/calculate")
                .header("Authorization", "Bearer $token")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"buildingId": 2, "year": 2026, "month": 3}""")
        )
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.success").value(true))
            .andExpect(jsonPath("$.data.buildingName").value("스타빌"))
            .andExpect(jsonPath("$.data.period.start").value("2026-03-01"))
            .andExpect(jsonPath("$.data.period.end").value("2026-03-31"))
            .andExpect(jsonPath("$.data.config.feeType").value("pct"))
            .andExpect(jsonPath("$.data.roomSettlements").isArray)
            .andExpect(jsonPath("$.data.deductions").isArray)
            .andExpect(jsonPath("$.data.deductions.length()").value(4))
            .andExpect(jsonPath("$.data.summary.totalRent").isNumber)
            .andExpect(jsonPath("$.data.summary.totalFee").isNumber)
            .andExpect(jsonPath("$.data.summary.finalAmount").isNumber)
    }
